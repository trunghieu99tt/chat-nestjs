...
import { MAILER_EMAIL_ID, MAILER_PASSWORD } from '../common/config/secrets'
...
class Mailer {
    ...
    sendMail(receiver: string, subject: string, text: string, senderName = "Rikikudo Support"): void {
        ...
        this.transporter.sendMail(mailOptions, (err: Error, info: SMTPTransport.SentMessageInfo) => {
            if (err) {
                console.error(err);
            } else {
    ...
    async sendMails(receivers: string[], subject: string, text: string): Promise<void> {
        const mailOptions: Mail.Options = {
